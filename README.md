...
Requires dekopon ≥ 0.22.0 (broker and catalog); console 0.1.0 is the last release for 0.19.
...
The published model clients support a private ChatGPT subscription `--auth-file` (`chatgpt-auth.console.json` by default), or `--endpoint URL` plus optional `--api-key-env NAME` for OpenAI-compatible endpoints. A named but absent/empty bearer variable refuses **when a model turn is attempted**, not while browsing. The console credential must not share the gateway's rotating refresh-token family; explicit `--auth-file` overrides the shared-file guard at the operator's risk. The chart should reference a separately owned API-key Secret or isolated subscription credential state, never mount broker/gateway credentials. No model is contacted for catalog browsing, empty grants or idle. `--server-uid` defaults to local effective UID only for same-owner development; the separate console UID 65535 / broker UID 65532 deployment must pass 65532 and have IPC group 65534, without write access to the socket parent.

The UI opens the selected agent's Shell ready to type, with Agents / Shell / Capabilities tabs and one wrapped command/output/input transcript. PageUp/PageDown page visual rows even while typing; Home/End jump to top/bottom (usually Fn-arrow keys on compact Mac keyboards, if the terminal forwards them). Esc requests cooperative stop while busy. There is no Turns or inference UI entry; underlying model/session machinery remains in the library. The retained session library uses the released shared prompt loop, mounted `read_skill`, safe `inspect_agent_config` and progress metadata; the shell UI uses broker-granted provider command words and help. Cooperative cancellation reaches manual shell commands but does **not** undo effects already accepted. The capability pane shows the current broker leg’s W3C trace ID; on submission the fresh leg adopts the console turn’s trace. Each manual command creates a fresh console root; shell, broker and provider spans inherit it through blocking work. Without export configured this is local context correlation only, not durable telemetry evidence. Manual shell arguments/results appear in the terminal transcript; output and command text are control-sanitised. Fresh agent entry resets transcript, shell state and model history. Manual scripts are Dekopon's bounded interpreter, not OS shell. It does not synthesize inbound delivery, chat replies, production transcript replay, or conversation memory. `suggest_improvement` remains deliberately off (not enabled by the approved session contract); OpenRouter models are not wired into the console. Text-only console inputs have no local asset source; `chat-asset:<N>` proposals are refused before broker submission. On provider asset effects, core drops returned descriptors and reports a note; the console turns that outcome into an explicit **failed local presentation** warning after the effect executed, never reports fake delivery. Binary asset intake/presentation is a follow-up, not parity. This release must not be marketed as full image or transport replay.

## Dependency / verification boundary

Exact crates.io `=0.22.0` Dekopon pins, Rust 1.98.1, and committed Cargo.lock. `DEKOPON_CONFIG`/`--config` may name a catalog file or a directory of `*.yaml` agents (the chart's ConfigMap mount), with `instructionsFile` read beside each. To exercise the broker boundary without live requests, set `DEKOPON_TEST_BROKERD` to a separately verified broker executable and `DEKOPON_TEST_PROBE_WASM` to core's pure `cli-probe-provider.wasm`, then run `cargo test -p dekopon-tui --test broker --locked`. The test skips only when those variables are absent; a submitted compatibility claim must provide them and record the artifact/ref tested. It tests attested/denied/empty surfaces, UID pin, policy-narrowed scope mismatch, provider help, pure read, ungranted call, and typed secret-use denial. Console 0.2.0 passed it against the released v0.22.0 `aarch64-apple-darwin` `dekopon-brokerd` with the v0.22.0 `cli-probe` and `http-probe` components. This proves the tested broker wire/fixture paths, not untested provider effects. No path/git dependency is committed.
